...
from .mocks import (
    ...
    CENSYS_SEARCH_JSON,
    CENSYS_TEST_HOST,
    CENSYS_TEST_SEARCH_QUERY,
    SHODAN_HOST_JSON,
    SHODAN_SEARCH_JSON,
    SHODAN_TEST_HOST,
    SHODAN_TEST_SEARCH_QUERY,
...
class TestQueryResult:
    def test_censys_host_source(self):
        query_result = QueryResult(
            raw_query_result=CENSYS_HOST_JSON,
            query_command="host",
            search_term=CENSYS_TEST_HOST,
        )
    ...
    def test_censys_search_source(self):
        query_result = QueryResult(
            raw_query_result=CENSYS_SEARCH_JSON,
            query_command="generic",
            search_term=CENSYS_TEST_SEARCH_QUERY,
        )
        assert query_result.source is CensysSourceConnector
    ...
    def test_shodan_host_source(self):
        query_result = QueryResult(
            raw_query_result=SHODAN_HOST_JSON,
            query_command="host",
            search_term=SHODAN_TEST_HOST,
        )
    ...
    def test_shodan_search_source(self):
        query_result = QueryResult(
            raw_query_result=SHODAN_SEARCH_JSON,
            query_command="generic",
            search_term=SHODAN_TEST_SEARCH_QUERY,
        )
        assert query_result.source is ShodanSourceConnector
...
class TestQuerying:
    ...
    def test_host_search_shodan(self):
        mock_shodan = MockShodanSourceConnector()
        query_result_pt1, query_result_pt2 = Querying.host_query(
            search=SHODAN_TEST_SEARCH_QUERY, connection=mock_shodan
        )
    ...
    def test_host_censys(self):
        ...
        query_result_pt1, query_result_pt2 = Querying.host_query(
            search=CENSYS_TEST_SEARCH_QUERY, connection=mock_censys
        )
        assert isinstance(query_result_pt1, QueryResult)
        assert query_result_pt2 is None
