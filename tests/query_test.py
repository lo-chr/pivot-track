...
class TestQuerying:
    def test_host_none_connector(self):
        with pytest.raises(NotImplementedError) as e:
            Querying.host(config = {}, host = "1.2.4.4", source = None)
        assert str(e.value) == "Did not find HostQuery connector."
    
    def test_host_wrong_connector_class(self):
        with pytest.raises(NotImplementedError) as e:
            Querying.host(config = {}, host = "1.2.4.4", source = OpenSearchConnector)
        assert str(e.value) == "Did not find HostQuery connector."

    def test_host_wrong_connector_string(self):
        with pytest.raises(NotImplementedError) as e:
            Querying.host(config = {}, host = "1.2.4.4", source = "ShodanSourceConnector")
        assert str(e.value) == "Did not find HostQuery connector."